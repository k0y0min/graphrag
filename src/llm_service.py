...
class LocalHuggingFaceBackend(LLMBackend):
    def __init__(self, model_id: str = "gpt2", device: str = None, local_dir: str = "models"):
        ...
        # Check if local model exists in models/ dir? 
        # For now, we rely on HF cache or download to local_dir if specific logic added.
        # User asked: "store downloaded models in a local folder and check if present"
        
        self.device = device or ("mps" if torch.backends.mps.is_available() else "cpu")
        if torch.cuda.is_available(): self.device = "cuda"
        
        logger.info(f"Loading Local HF Model: {model_id} on {self.device}")
        
        # We could set cache_dir=local_dir 
...
class OllamaBackend(LLMBackend):
    ...
    def check_causal_link(self, context: str, sentence: str) -> float:
        # Use embedding similarity since logits are hard to get from Ollama API
        ctx_emb = self.embed(context)
        sent_emb = self.embed(sentence)
        
        if not ctx_emb or not sent_emb:
            return 0.5
            
        # Cosine similarity
        import numpy as np
        vec_a = np.array(ctx_emb)
        vec_b = np.array(sent_emb)
